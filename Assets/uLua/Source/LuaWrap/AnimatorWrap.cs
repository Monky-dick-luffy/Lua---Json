...
public class AnimatorWrap
{
	public static void Register(IntPtr L)
	{
		LuaMethod[] regs = new LuaMethod[]
		{
			...
			new LuaMethod("GetCurrentAnimatorClipInfoCount", GetCurrentAnimatorClipInfoCount),
			new LuaMethod("GetCurrentAnimatorClipInfo", GetCurrentAnimatorClipInfo),
			new LuaMethod("GetNextAnimatorClipInfoCount", GetNextAnimatorClipInfoCount),
		...
		LuaField[] fields = new LuaField[]
		{
			...
			new LuaField("hasBoundPlayables", get_hasBoundPlayables, null),
			new LuaField("avatar", get_avatar, set_avatar),
			new LuaField("playableGraph", get_playableGraph, null),
	...
	static int get_isInitialized(IntPtr L)
	{
		...
		LuaScriptMgr.Push(L, obj.isInitialized);
		return 1;
	}
	...
	static int get_parameterCount(IntPtr L)
	{
		...
		LuaScriptMgr.Push(L, obj.parameterCount);
		return 1;
	}
	...
	static int get_hasBoundPlayables(IntPtr L)
	{
		...
		if (obj == null)
		{
			...
			if (types == LuaTypes.LUA_TTABLE)
			{
				LuaDLL.luaL_error(L, "unknown member name hasBoundPlayables");
			}
			else
			{
				LuaDLL.luaL_error(L, "attempt to index hasBoundPlayables on a nil value");
			}
		}

		LuaScriptMgr.Push(L, obj.hasBoundPlayables);
		return 1;
	}
	...
	static int get_playableGraph(IntPtr L)
	{
		...
		if (obj == null)
		{
			...
			if (types == LuaTypes.LUA_TTABLE)
			{
				LuaDLL.luaL_error(L, "unknown member name playableGraph");
			}
			else
			{
				LuaDLL.luaL_error(L, "attempt to index playableGraph on a nil value");
			}
		}

		LuaScriptMgr.PushValue(L, obj.playableGraph);
		return 1;
	}
	...
	static int GetCurrentAnimatorClipInfo(IntPtr L)
	{
		...
		else if (count == 3)
		{
			Animator obj = (Animator)LuaScriptMgr.GetUnityObjectSelf(L, 1, "Animator");
			int arg0 = (int)LuaScriptMgr.GetNumber(L, 2);
			List<AnimatorClipInfo> arg1 = (List<AnimatorClipInfo>)LuaScriptMgr.GetNetObject(L, 3, typeof(List<AnimatorClipInfo>));
			obj.GetCurrentAnimatorClipInfo(arg0,arg1);
			return 0;
		}
		else
		{
			LuaDLL.luaL_error(L, "invalid arguments to method: Animator.GetCurrentAnimatorClipInfo");
		}
	...
	static int GetNextAnimatorClipInfo(IntPtr L)
	{
		...
		{
			LuaDLL.luaL_error(L, "invalid arguments to method: Animator.GetNextAnimatorClipInfo");
		}
